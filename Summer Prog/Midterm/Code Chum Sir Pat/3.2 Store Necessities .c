...
void expiredProductRemoval(storeInventory *store, date currentDate) {
    int i, j;
    float lossValue = 0.0;
    int hasExpiredItems = 0; // Flag to check if any items have been moved to expiredInventory
    ...
    for (i = 0; i < store->currentInventory.inventoryCount; i++) {
        if (store->currentInventory.product[i].expiryDate.year < currentDate.year ||
            ...
            hasExpiredItems = 1; // Set flag indicating items have been moved to expiredInventory
        }
    }
    ...
    if (hasExpiredItems) {
        ...
        // Display loss value for items moved to expiredInventory
        printf("\nLoss Value: %.2f\n", lossValue);
    } else {
        // No items were moved to expiredInventory
        printf("\nNo items were expired.\n");
    }
}
